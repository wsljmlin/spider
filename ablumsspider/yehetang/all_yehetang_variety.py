...
class all_yehetang_variety():
    def __init__(self):
        print "Do spider all_yehetang_variety."
        self.program = copy.deepcopy(BASE_CONTENT["program"])
        self.program_sub = PROGRAM_SUB
        self.seedBase = ["http://www.yehetang.com/pd/3-1.html"
                         , "http://www.yehetang.com/pd/3-1.html"
                         , "http://www.yehetang.com/pd/3-2.html"
                         , "http://www.yehetang.com/pd/3-3.html"
                         , "http://www.yehetang.com/pd/3-4.html"
                         , "http://www.yehetang.com/pd/3-5.html"
                         , "http://www.yehetang.com/pd/3-6.html"
        ]
        self.seedList = []
        self.dataDir = '.' + os.path.sep + 'data'
        self.today = time.strftime('%Y%m%d', time.localtime())
        self.dataFile = spiderTool.openFile(self.dataDir + os.path.sep + "all_yehetang_variety_" + self.today + ".txt")
    ...
    def secondSpider(self, video_list):
        ...
        for video in video_list:
            ...
            elif re.search(r'(\d+)期'.decode('utf8'), setName):
                setNumber = re.search(r'(\d+)期'.decode('utf8'), setName).group(1)
            elif re.search(r'(\d+)'.decode('utf8'), setName):
                setNumber = re.search(r'(\d+)'.decode('utf8'), setName).group(1)
            elif len(video_list)==1 and len(videoinfo_list)==1:
                setNumber = "1"
            ...
            elif re.search(r'(\d+)期'.decode('utf-8'), videoinfo_tmp.decode('utf-8')):
                videoinfo_num = re.search(r'(\d+)期'.decode('utf-8'), videoinfo_tmp.decode('utf-8')).group(1)
                if videoinfo_num == setNumber:
                    parm = urllib.quote(videoinfo_tmp)
                    webUrl = "%s###%s" % (webUrl, parm)
